...
keynor-core is both **Authorization Server** and **Resource Server**, with three roles (`ADMIN`, `DEFAULT` — added 2026-08-05 for keynor-rpg's player login, see below — and `SYSTEM`) and 3 ordered `@Order` filter chains. No default users or clients exist in the schema — they must be inserted manually before the API can be used.
...
**Exception — closed reference/lookup sets:** `Era`, `Archetype`, and `Sign` are not `UniverseEntity` subclasses. They have no `status` field (so the CANON-only filter does not apply) and are fixed, non-paginated sets, so their list endpoints return a plain JSON array of all entries instead of `PagedResponse<T>`, and accept no `categories`/`page`/`size` query parameters. **They still require `language`**, same as every other list endpoint — see the row below. See `.claude/skills/domain-entity-reference.md`.
...
|-----------|------|---------|-------------|
| `language` | `String` | **none — required** | `EN` or `PT` (case-insensitive, parsed via `LanguageRequestParser`). Every `findAll`/list endpoint rejects the request with `400` if this is missing — there is no default. Applies to all 6 `UniverseEntity` list endpoints **and** to `Era`, `Archetype`, `Sign` (public and internal alike). **Not** required on any `findById` endpoint — a single row's id already pins its language, so no filter is needed there. |
| `categories` | `List<String>` | — | Filter by one or more category values |
| `page` | `int` | `0` | Zero-based page number |
| `size` | `int` | `20` | Page size |

**`language` is GET-only, not a body field on update:** `Update*Request` DTOs never carry `language` — a row's language is fixed at creation and cannot be changed via `PUT`. To add the other language, submit a new row via `Create*Request` with `translationGroupId` set to the existing row's id (see `.claude/skills/domain-entity-reference.md` — "Multilingual fields"). Don't confuse the two: `language` is a **query parameter** on every list GET, and a **request-body field** on every `Create*Request` — it never appears on `Update*Request` or on any `findById` call.
...
A visibility dimension independent of `status` and orthogonal to `hidden` (V19, 2026-08-12) — a "common" entity is excluded from every public list/browse endpoint and from every map pin, the same as `hidden`, but with **no lock**: no riddle, no password, no unlock token. `findById` and links-resolution stay open and unredacted for a common entity — the only route to it in the UI is a link from another already-visible entity ("accessed via references from other elements"). Unlike `hidden`, a common entity carries no status invariant — it may be `DRAFT`, `CANON`, or `DEPRECATED`.

Wired for all 6 entity types. Since aniannoth-overview has no local copy of universe data (every list/pin view is a direct render of the public API response), excluding `common` entities at these two endpoints was sufficient on its own — no frontend change was required for this feature. For the full schema and domain wiring, see `.claude/skills/common-content-implementation.md`.
...
`docs/lore-todo.md` is a plain-text checklist (grouped by category — Character, Place, Faction, Item, Event, Lore) of universe elements mentioned by already-submitted content that do not yet exist in keynor-core. It carries no sensitive data and needs no protection — edited directly, no endpoint. Aroneus maintains it as part of the normal content-authoring workflow (see `.claude/agents/aroneus.md` — "Lore gap tracking" for the full resolution procedure, including a Skill 13 scoped exception letting Aroneus self-merge a PR when this file is the only thing it changes).
...
*Last updated: 2026-08-26 (task/aroneus-lore-todo-tracking, Omnia) — added the "Lore gap tracking" section documenting the new `docs/lore-todo.md` file and cross-referencing `aroneus.md` for the full procedure and its Skill 13 scoped self-merge exception. Previous entry, 2026-08-12 (task/add-common-content-flag: added the "Common content" section documenting the new `common` visibility flag (V19) — excluded from public list/browse and map pins like `hidden`, but with no lock/riddle/password and no status invariant; wired for all 6 entity types alongside the new `.claude/skills/common-content-implementation.md`. Scoped initially as a Lamont cross-project task, then routed to Imaws alone once investigation showed aniannoth-overview needs no code change — it has no local data copy, so excluding `common` entities from the two relevant keynor-core public endpoints was sufficient on its own.) Previous entry, 2026-08-05 (task/add-default-role-and-rpg-client, cross-project decision with the user, escalated via Omnia: added `UserRole.DEFAULT` — a plain enum addition, no migration needed since `users.role` is an unconstrained `VARCHAR(20)` — so keynor-rpg can authenticate regular players against this same Authorization Server, reusing the `ADMIN` human-login flow's exact mechanism (`authorization_code`+PKCE, `role` claim, `UserDetailsServiceImpl`) with no code change to any of that machinery. Also corrected two unrelated stale facts found while touching `.claude/skills/security-model.md` for this: the bootstrap "Insert the ADMIN user" SQL example used `email`/`active` columns that were never real (`V1__create_users.sql` has `username`/`enabled`, plus required `created_at`/`updated_at`) and would have failed if actually run; and the CORS section mislabeled `http://localhost:5173` as `aniannoth-overview`'s origin when that project actually moved to a fixed port 4173 specifically to avoid colliding with `keynor-rpg-client`'s own default use of 5173. Added a new bootstrap step documenting how to register a browser PKCE client at all — previously undocumented, `aniannoth-overview`'s own `aniannoth-admin` client was flagged in that project's `CLAUDE.md` as unregistered with no worked example to follow. Root `ARCHITECTURE.md` also corrected in the same effort (separate PR, Omnia) — its Hidden Content section described this flow as tied to `inputter`/`reader` roles, which never existed in this codebase.) Previous entry, 2026-08-01 (corrected the "Controller structure" diagram's stale claim that Era had "no internal controller yet" — `InternalEraController` already existed for `create`, and PR #114 added `update`; now lists it with its current scope. Previous entry, 2026-07-31: documented `language` as a required query parameter on every list/`findAll` endpoint — public and internal, all 6 `UniverseEntity` types plus Era/Archetype/Sign — in the "Query parameters (list endpoints)" table; corrected the Era/Archetype/Sign exception note, which previously and incorrectly claimed those endpoints "accept no query parameters" when they in fact require `language`; clarified that `language` never appears on `Update*Request` or `findById` calls. Prompted by a real gap: an agent made a `GET` list-endpoint call without `language` because no project document stated the requirement, even though it was already documented as a `Create*Request` body field. Previous entry, 2026-07-30: new Level 1 agent Dixia — Primordial Era historian, narrower-than-baseline scope, carrying a named read-only/unlimited-row database exception modeled on Jung's migration exception and Siegmund's `pg_dump` exception. Updated the Agent structure diagram and table accordingly. Previous entry, 2026-07-26: hidden content is now wired for all 6 entity types, not just Character/Lore — updated the "Hidden content" section accordingly. Previous entry, 2026-07-24: added the "Hidden content" section documenting `hidden`/`hidden_content_lock` and the one-way linking rule, alongside the new `.claude/skills/hidden-content-implementation.md` — see keynor-core PR #88. Previous entry, 2026-07-15: documented the `GET /api/public/v1/characters/batch` endpoint as the second exception to the CANON-only public API invariant, alongside the pre-existing Era/Archetype/Sign closed-set exception — see keynor-core PR #71)*
