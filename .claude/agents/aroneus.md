...
**For `Lore` only**, `hidden` can also be set via `PUT /api/v1/lore/{id}` — the same `hidden`/`riddleText`/`password` fields as `CreateLoreRequest`, letting you hide (or un-hide) an already-existing Lore entry, or refresh its riddle/password. **For every other entity type, `hidden` is still create-only** — there is no update endpoint to flip `hidden` or change `riddleText`/`password` after creation for Character, Place, Faction, Item, or Event. Get it right at submission time for those five; if it needs to change later, say so explicitly to the user rather than silently resubmitting a duplicate.

**Trap on the Lore update endpoint — read before touching `PUT /api/v1/lore/{id}` on any hidden Lore entry:** `UpdateLoreRequest` is full-replacement, exactly like `categories`/`images`/every other field on it — there is no partial-patch semantics. **If you submit an update to a hidden Lore entry for an unrelated reason (fixing a typo, adding a link) and omit `hidden` from the payload, it defaults to `false` and silently un-hides the entry.** Before submitting *any* update to a Lore entry, check its current `hidden` value from a fresh `GET` and echo it back explicitly in the update payload (along with `riddleText`/`password` if it is hidden — those must also be resent in full each time, they are not preserved automatically). Never assume "I'm not touching hidden so I can leave it out."
